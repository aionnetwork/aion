...
int blake2b_init_key( blake2b_state *S, size_t outlen, const void *key, size_t keylen, bool fastPath )
{
  blake2b_param P[1];
...

  {
    ...
    blake2b_update( S, block, BLAKE2B_BLOCKBYTES , fastPath);
    secure_zero_memory( block, BLAKE2B_BLOCKBYTES ); /* Burn the key from stack */
  }
  return 0;
}

static void blake2b_compress( blake2b_state *S, const uint8_t block[BLAKE2B_BLOCKBYTES], bool shortPath)
{
  __m128i row1l, row1h;
  __m128i row2l, row2h;
  __m128i row3l, row3h;
...
int blake2b_update( blake2b_state *S, const void *pin, size_t inlen , bool fastPath)
{
  ...
  if( inlen > 0 )
  {
    ...
    if( inlen > fill )
    {
      ...
      blake2b_compress( S, S->buf , fastPath); /* Compress */
      in += fill; inlen -= fill;
      while(inlen > BLAKE2B_BLOCKBYTES) {
        blake2b_increment_counter(S, BLAKE2B_BLOCKBYTES);
        blake2b_compress( S, in, fastPath );
...
int blake2b_final( blake2b_state *S, void *out, size_t outlen, bool shortPath )
{
  ...
  blake2b_compress( S, S->buf , shortPath);
...
int blake2b( void *out, size_t outlen, const void *in, size_t inlen, const void *key, size_t keylen, bool fastPath )
{
  blake2b_state S[1];
  ...
  if( keylen )
  {
    if( blake2b_init_key( S, outlen, key, keylen, fastPath ) < 0 ) return -1;
  }
  else
  {
  ...
  blake2b_update( S, ( const uint8_t * )in, inlen, fastPath );
  blake2b_final( S, out, outlen, fastPath );
  return 0;
}

int blake2( void *out, size_t outlen, const void *in, size_t inlen, const void *key, size_t keylen ) {
  return blake2b(out, outlen, in, inlen, key, keylen, false);
}
