...
public interface Blake2b {
	...
	class Param implements AlgorithmParameterSpec {
		...
		public final Param setSalt(final byte[] salt) {
			...
			assert salt.length <= Spec.MAX_SALT : assertFail("salt.length", salt.length, inclusiveUpperBound,
					Spec.MAX_SALT);
		...
		public final Param setPersonal(byte[] personal) {
			...
			assert personal.length <= Spec.MAX_PERSONALIZATION : assertFail("personal.length", personal.length,
