...
import static org.aion.crypto.hash.Blake2b.Engine.Assert.*;
import static org.aion.crypto.hash.Blake2b.Engine.LittleEndian.*;
...
public interface Blake2b {
    ...
    class Mac extends Engine implements Blake2b {
        ...
        /**
         * Blake2b.MAC - using default Blake2b.Spec settings with given key,
         * with given digest length
         */
    ...
    // ---------------------------------------------------------------------
    /**
     * Note that Tree is just a convenience class; incremental hash (tree) can
     * be done directly with the Digest class.
     * <br>
     * Further node, that tree does NOT accumulate the leaf hashes -- you need
     * to do that
     */
    class Tree {
         ...
         * @param inner_length note this is used also as digest-length for
         * non-root nodes.
...
//        }
        ...
        // ---------------------------------------------------------------------
        /**
         * write out the digest output from the 'h' registers. truncate full
         * output if necessary.
         */
...
//			Debug.dumpArray("h @ compress end", h);
 ...
 /* def. personal is 0 fill and already set by new byte[] */
        ...
        public final Param setDigestLength(int len) {
            ...
            assert len <= Spec.MAX_DIGEST : assertFail("len", len, inclusiveUpperBound, Spec.MAX_DIGEST);
        ...
        public final Param setKey(final byte[] key) {
            ...
            assert key.length <= Spec.MAX_KEY : assertFail("key.length", key.length, inclusiveUpperBound, Spec.MAX_KEY);
 ...
 /* 32-47 inclusive */
        public final Param setSalt(final byte[] salt) {
            ...
            assert salt.length <= Spec.MAX_SALT : assertFail("salt.length", salt.length, inclusiveUpperBound, Spec.MAX_SALT);
        ...
        public final Param setPersonal(byte[] personal) {
            ...
            assert personal.length <= Spec.MAX_PERSONALIZATION : assertFail("personal.length", personal.length, inclusiveUpperBound, Spec.MAX_PERSONALIZATION);
