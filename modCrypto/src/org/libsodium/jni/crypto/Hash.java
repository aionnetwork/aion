...
public class Hash {
    ...
    public String pwhash_scryptsalsa208sha256(String passwd, Encoder encoder, byte[] salt,
        int opslimit, int memlimit) {
        buffer = new byte[KEY_LEN];
        sodium();
        Sodium.crypto_pwhash_scryptsalsa208sha256(buffer, buffer.length, passwd.getBytes(),
            passwd.length(), salt, opslimit, memlimit);
    ...
    public byte[] blake2(byte[] message) throws UnsupportedOperationException {
        ...
        sodium();
        Sodium.crypto_generichash_blake2b(buffer, SodiumConstants.BLAKE2B_OUTBYTES, message,
