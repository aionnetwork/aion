...
public class EventMgrA0 extends AbstractEventMgr implements IEventMgr {
        ...
        synchronized (this) {
            for (IEvent e : _evt) {
                if (LOG.isTraceEnabled()) {
                    LOG.trace("EVTMGR.registerEvent EventType [{}] CallbackType [{}]",
                        e.getEventType(),
                        e.getCallbackType());
                }
    ...
    public boolean newEvents(List<IEvent> _evt) {
        for (IEvent e : _evt) {
            ...
            if (hdr == null) {
                ...
                if (LOG.isTraceEnabled()) {
                    LOG.trace("EVTMGR.newEvents eCBT:[{}] eEVT:[{}]", e.getCallbackType(),
                        e.getEventType());
                }
    ...
    public boolean newEvent(IEvent _evt) {
        ...
        if (hdr == null) {
            ...
            if (LOG.isTraceEnabled()) {
                LOG.trace("EVTMGR.newEvent eCBT:[{}] eEVT:[{}]", _evt.getCallbackType(),
                    _evt.getEventType());
            }
