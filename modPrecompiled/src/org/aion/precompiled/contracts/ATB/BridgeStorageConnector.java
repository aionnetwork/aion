 ...
 *
 * ------------------------------------------------------------ owner            DWORD   0x0 current
 * owner of the contract newOwner         DWORD   0x1     proposed new owner of the contract
 *
 * memberCount      WORD    0x2     the total amount of members minThresh        WORD    0x3     the
 * minimum amount of votes required ringLocked       WORD    0x4     if the ring is locked or not
 *
 * There are two mapping fields: memberMap        map[WORD] => boolean    {@code TRUE} if member is
 * active bundleMap        map[WORD] => boolean    {@code TRUE} if bundle has been processed
 *
 * Maps are prepended with respective offsets, their keys are derived as the following:
 *
 * {@code h256(concat(MAP_OFFSET, key))}
 *
 * Handles all loading/unloading, disguises itself like a simple POJO from the outside.
 *
 * Impl Detail Notes (Contracts):
 *
 * [C1] The storage does not distinguish between empty byte array (16 bytes 0) and nulls. Any time
 * it detects that the response is a 16-byte 0, a null is returned instead.
 */
public class BridgeStorageConnector {
     ...
     *
     * Therefore, where previously we were checking whether the bundle was valid based on a {@code
     * true/false} assumption, we now check whether the bundle is valid based on whether the
     * returned address equates to a zero word.
     *
     * Documentation on the change can be found as part of v0.0.4 changes.
     */
