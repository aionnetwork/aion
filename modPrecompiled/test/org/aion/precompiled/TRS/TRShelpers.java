...
class TRShelpers {
    ...
    AionAddress getNewExistentAccount(BigInteger balance) {
        AionAddress acct = new AionAddress(ECKeyFac.inst().create().getAddress());
        acct.toByteArray()[0] = (byte) 0xA0;
    ...
    byte[] getDepositForInput(AionAddress contract, AionAddress beneficiary, BigInteger amount) {
        ...
        input[0] = 0x5;
        System.arraycopy(contract.toByteArray(), 0, input, 1, AionAddress.LENGTH);
        System.arraycopy(beneficiary.toByteArray(), 0, input, 33, AionAddress.LENGTH);
    ...
    byte[] getRefundInput(AionAddress contract, AionAddress account, BigInteger amount) {
        ...
        input[0] = 0x4;
        System.arraycopy(contract.toByteArray(), 0, input, 1, AionAddress.LENGTH);
        System.arraycopy(account.toByteArray(), 0, input, 33, AionAddress.LENGTH);
        System.arraycopy(amtBytes, 0, input, 193 - amtBytes.length, amtBytes.length);
        return input;
    }
    ...
    byte[] getPeriodAtInput(AionAddress contract, long blockNum) {
        ...
        buffer.put((byte) 0x4);
        buffer.put(contract.toByteArray());
    ...
    byte[] getBulkDepositForInput(AionAddress contract, AionAddress[] beneficiaries, BigInteger[] amounts) {
        ...
        int index = 33;
        for (int i = 0; i < len; i++) {
            System.arraycopy(beneficiaries[i].toByteArray(), 0, input, index, AionAddress.LENGTH);
            index += 32 + 128;
