...
public interface IDatabase {
     ...
     * <p>
     * Implements the "CREATE_IF_NOT_EXISTS" functionality by default (non-configurable in this
     * spec).
     * <p>
     * DB file(s) on disk policy: all DBs shall be enclosed in a directory, regardless of the
     * underlying implementation being a single file store or a multi-file store, to make DB cleanup
     * easiers
     * <p>
     * Does NOT throw an exception on failure, simply returns false and logs reason for failure.
     ...
     * <p>
     * NOTE: All interface calls on an instance for which [isClosed() == true] will throw a Runtime
     * Exception
     */
    void close();
     ...
     * persistent DB (assuming all driver implementations create paths on filesystem such that all
     * DB files, whether single file or a multi-file implementation)
     * <p>
     ...
     * <p>
     * Implementation note: can't just try to open a connection to see if it's successful since a
