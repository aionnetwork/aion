...
public class TaskConnectPeers2 implements Runnable {
    ...
    public TaskConnectPeers2(P2pMgr _mgr, AtomicBoolean _start, boolean _showLog, NodeMgr _nodeMgr,
        int _maxActiveNodes, Selector _selector, LinkedBlockingQueue<MsgOut> _sendMsgQue,
    ...
    public void run() {
        ...
        while (this.start.get()) {
            ...
            if (this.nodeMgr.activeNodesSize() >= this.maxActiveNodes) {
                if (this.showLog) System.out.println("<p2p-tcp-connect-peer pass max-active-nodes>");
                continue;
            }
