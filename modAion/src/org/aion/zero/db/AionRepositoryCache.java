...
public class AionRepositoryCache extends AbstractRepositoryCache<IBlockStoreBase<?, ?>> {
     ...
     * flush()} method. The changes are propagated to the parent through calling the parent's {@code
     * updateBatch()} method.
     */
    @Override
    public void flush() {
        ...
        try {
            ...
            for (Map.Entry<Address, IContractDetails<IDataWord>> entry : cachedDetails.entrySet()) {
                ...
                if (ctd != null && ctd instanceof ContractDetailsCacheImpl) {
                    ...
                    if (contractDetailsCache.origContract == null && repository
                        .hasContractDetails(entry.getKey())) {
                        ...
                        contractDetailsCache.origContract = repository
                            .getContractDetails(entry.getKey());
                        contractDetailsCache.commit();
                    }
                }
            }
    ...
    @Override
    public void updateBatch(Map<Address, AccountState> accounts,
        ...
        try {
            ...
            for (Map.Entry<Address, IContractDetails<IDataWord>> ctdEntry : details.entrySet()) {
                ContractDetailsCacheImpl contractDetailsCache = (ContractDetailsCacheImpl) ctdEntry
                    .getValue();
                if (contractDetailsCache.origContract != null
                    ...
                    cachedDetails.put(ctdEntry.getKey(), ContractDetailsCacheImpl
                        .copy((ContractDetailsCacheImpl) contractDetailsCache.origContract));
                } else {
                    // Either no parent or we have Repo's AionContractDetailsImpl, which should be flushed through RepoImpl
                    cachedDetails.put(ctdEntry.getKey(),
                        ContractDetailsCacheImpl.copy(contractDetailsCache));
                }
            }
        } finally {
            fullyWriteUnlock();
