...
public final class BroadcastTxHandler extends Handler {
    ...
    public BroadcastTxHandler(final Logger _log, final IPendingStateInternal _pendingState,
    ...
    public final void receive(int _nodeIdHashcode, String _displayId, final byte[] _msgBytes) {
        ...
        if (broadCastTx == null) {
            log.error("<BroadcastTxHandler decode-error unable to decode tx-list from {}, len: {]>",
                _displayId, _msgBytes.length);
    ...
    private List<AionTransaction> castRawTx(List<byte[]> broadCastTx) {
        ...
        if (log.isTraceEnabled()) {
            log.trace("BroadcastTxHandler.castRawTx Tx#{} validTx#{}", broadCastTx.size(),
                rtn.size());
        }
