 ...
 * trusted peers, proven by pow headers validations. Keep some degree of tolerance based on common
 * best block. Process is driven by network status update 2. maintain track points on re-sync
 * process
 */
public class SyncAid {
    ...
    public void update(int _nodeId, final List<A0BlockHeader> _latestHeaders) {
        ...
        for (int i = 0; i < HEADERS_LEN; i++) {
            if (i > 0 && _latestHeaders.get(i).getNumber() != (_latestHeaders.get(i - 1).getNumber()
                + 1)) {
                ifSequential = false;
            }
        }
