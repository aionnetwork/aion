...
public class AionImpl implements IAionChain {
    ...
    private AionImpl() {
        ...
        aionHub = new AionHub();
        LOG_GEN.info(
            "<node-started endpoint=p2p://" + cfg.getId() + "@" + cfg.getNet().getP2p().getIp()
                + ":"
    ...
    @Override
    public AionTransaction createTransaction(BigInteger nonce, Address to, BigInteger value,
        byte[] data) {
    ...
    public long estimateTxNrg(AionTransaction tx, IAionBlock block) {
        ...
        IRepositoryCache repository = aionHub.getRepository().getSnapshotTo(block.getStateRoot())
            .startTracking();
    ...
    public boolean isSyncComplete() {
        try {
            long localBestBlockNumber = this.getAionHub().getBlockchain().getBestBlock()
                .getNumber();
            long networkBestBlockNumber = this.getAionHub().getSyncMgr()
                .getNetworkBestBlockNumber();
    ...
    public Optional<AccountState> getAccountState(Address address, long blockNumber) {
        try {
            byte[] stateRoot = this.aionHub.getBlockStore().getChainBlockByNumber(blockNumber)
                .getStateRoot();
            AccountState account = (AccountState) this.aionHub.getRepository()
    ...
    public Optional<AccountState> getAccountState(Address address, byte[] blockHash) {
        try {
            byte[] stateRoot = this.aionHub.getBlockchain().getBlockByHash(blockHash)
                .getStateRoot();
            AccountState account = (AccountState) this.aionHub.getRepository()
                .getSnapshotTo(stateRoot)
                .getAccountState(address);
