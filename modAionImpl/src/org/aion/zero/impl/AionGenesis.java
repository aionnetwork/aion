...
public class AionGenesis extends AionBlock {
     ...
     */
    protected static final Address NETWORK_BALANCE_ADDRESS = ContractFactory
        .getTotalCurrencyContractAddress();
     ...
     * estimate of the projected hashrate. The predicted difficulty number is: 189625700 (189M). It
     * is unrealistic to expect this amount during the test-net phase, therefore divide this number
     * by 100, this roughly accounts for ~440 GPUs, something attainable by the founation on
     * test-net launch.
     ...
     */
    protected static final byte[] GENESIS_DIFFICULTY = ByteUtil
        .bigIntegerToBytes(BigInteger.valueOf(1024));
     ...
     *
     * Previously, this was set to a low value, but for the test net we would like to stress the
     ...
     *
     * @see <a href= "https://blog.ethereum.org/2016/10/31/uncle-rate-transaction-fee-analysis">fee-analysis</a>
     */
    ...
    // completed)
    public AionGenesis(byte[] parentHash, Address coinbase, byte[] logsBloom, byte[] difficulty,
        long number,
        long timestamp, byte[] extraData, byte[] nonce, long energyLimit)
        throws HeaderStructureException {
        super(parentHash, coinbase, logsBloom, difficulty, number, timestamp, extraData, nonce,
            energyLimit);
    }
     ...
     * does not override them, note that because a block by default is not immutable we are required
     * here to create a new instance of the genesis each time the builder produces a new block.</p>
     *
     * <p>Does not assume anything about the input data, will do the necessary
     * checks to assert the specs. But will not do null checks, therefore it is up to the caller to
     * ensure input values are non null</p>
     *
    ...
    public static class Builder {
        ...
        protected byte[] generateRootHash() {
            ...
            for (Map.Entry<Integer, BigInteger> entry : this.networkBalance.entrySet()) {
                networkBalanceStorage
                    .put(new DataWord(entry.getKey()), new DataWord(entry.getValue()));
            }
            ...
            for (Map.Entry<Address, AccountState> preminedEntry : this.premined.entrySet()) {
                worldTrie.update(preminedEntry.getKey().toBytes(),
