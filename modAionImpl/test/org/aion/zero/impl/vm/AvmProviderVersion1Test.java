...
/**
 * Tests the {@link AvmProvider} class by making specific calls to version 1 of the avm.
 */
public class AvmProviderVersion1Test {
    private String projectRootDir;

    @Before
    public void setup() throws Exception {
        // Ensure we always begin with a disabled avm version 1.
    ...
    public static void tearDownClass() throws Exception {
        // Ensure we never exit with an enabled avm version 1.
        if (!AvmProvider.holdsLock()) {
            Assert.assertTrue(AvmProvider.tryAcquireLock(1, TimeUnit.MINUTES));
        }
        AvmProvider.disableAvmVersion(AvmVersion.VERSION_1);
    ...
    public void testGetAvmWithoutOwningLock() {
        AvmProvider.getAvm(AvmVersion.VERSION_1);
    }
    ...
    public void testNewExternalStateBuilderWithoutOwningLock() {
        AvmProvider.newExternalStateBuilder(AvmVersion.VERSION_1);
    }
    ...
    public void testNewExternalStateBuilderWhenEnabled() throws Exception {
        Assert.assertTrue(AvmProvider.tryAcquireLock(1, TimeUnit.MINUTES));
        AvmProvider.enableAvmVersion(AvmVersion.VERSION_1, this.projectRootDir);
        Assert.assertNotNull(AvmProvider.newExternalStateBuilder(AvmVersion.VERSION_1));
        AvmProvider.releaseLock();
    }
