...
public class BlockInfoTest {
    ...
    private byte[] OLD_BLOCKINFO_DATA_1 = ByteUtil.hexStringToBytes(
    ...
    public void testBlockInfoSerialization() {
        ...
        byte[] serialized = AionBlockStore.BLOCK_INFO_SERIALIZER
            .serialize(Collections.singletonList(info));
        System.out.println("serialized: " + new ByteArrayWrapper(serialized));

        List<AionBlockStore.BlockInfo> deserializedBlockInfos = AionBlockStore.BLOCK_INFO_SERIALIZER
    ...
    public void testBlockInfoMultipleSerialization() {
        ...
        byte[] serialized = AionBlockStore.BLOCK_INFO_SERIALIZER
            .serialize(Arrays.asList(info, info2));
        System.out.println("serialized: " + new ByteArrayWrapper(serialized));
    ...
    public void testBlockInfoMigrationSerialization() {
        ...
        List<AionBlockStore.BlockInfo> des = AionBlockStore.BLOCK_INFO_SERIALIZER
            .deserialize(serialized);
        ...
        assertThat(Arrays.equals(info.getHash(), ByteUtil.hexStringToBytes(
            "0xbfd84d03d8726c6b3b9221d2cce051b94900a2b1dd6186838a2bd9c47853e7a8"))).isTrue();
