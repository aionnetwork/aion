...
public class AccountManagerTest {
    ...
    public void testCtor() {
        ...
        assertThat(unit.getAccounts().stream().map(acc -> acc.getPublicAddress())
    ...
    public void testCreateMasterAccount() throws Exception {
        ...
        assertThat(unit.getRoot().getEcKey().getPubKey(),
            is(CryptoUtils.generateSha512HashedBip39ECKey(mnemonic).getPubKey()));
        assertThat(unit.getRoot().getEcKey().getPrivKeyBytes(),
    ...
    public void testUnlockMasterAccountWhenMasterAccountExists() throws Exception {
        ...
        MasterKey rootMasterKey = new MasterKey(
            CryptoUtils.generateSha512HashedBip39ECKey(mnemonic));
        ECKey derivedKey1 = rootMasterKey.deriveHardened(new int[]{44, 425, 0, 0, 0});
        String address1 = TypeConverter
            .toJsonHex(derivedKey1.computeAddress(derivedKey1.getPubKey()));
        ECKey derivedKey2 = rootMasterKey.deriveHardened(new int[]{44, 425, 0, 0, 1});
        String address2 = TypeConverter
            .toJsonHex(derivedKey2.computeAddress(derivedKey2.getPubKey()));
    ...
    public void testImportKeystoreWhenShouldNotKeepAccountAndAccountAlreadyExists()
        ...
        String pubAddressString = TypeConverter
            .toJsonHex(derivedKey1.computeAddress(derivedKey1.getPubKey()));
    ...
    public void testImportKeystoreWhenShouldKeepAccount() throws Exception {
        ...
        when(keystoreWrapper.create(eq(password),
            keystoreCreateECKeyCapture.capture() /* because ECKey doesn't have equals() */)
