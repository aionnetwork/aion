...
public class WalletStorage {
    ...
    public String getAccountName(final String address) {
        return Optional.ofNullable(accountsProperties.get(address + ACCOUNT_NAME_PROP))
            .map(Object::toString).orElse(BLANK);
    }
    ...
    public void setMasterAccountMnemonic(final String mnemonic, String password)
        ...
        try {
            if (mnemonic != null) {
                accountsProperties
                    .setProperty(MASTER_MNEMONIC_PROP, encryptMnemonic(mnemonic, password));
                saveWalletSettings();
            }
        } catch (Exception e) {
    ...
    public void incrementMasterAccountDerivations() {
        accountsProperties
            .setProperty(MASTER_DERIVATIONS_PROP, getMasterAccountDerivations() + 1 + "");
        saveAccounts();
    }
    ...
    private String decryptMnemonic(String encryptedMnemonic, String password) throws Exception {
        SecretKeySpec skeyspec = new SecretKeySpec(password.getBytes(),
            MNEMONIC_ENCRYPTION_ALGORITHM);
        Cipher cipher = Cipher.getInstance(MNEMONIC_ENCRYPTION_ALGORITHM);
        cipher.init(Cipher.DECRYPT_MODE, skeyspec);
        byte[] decrypted = cipher
            .doFinal(encryptedMnemonic.getBytes(MNEMONIC_STRING_CONVERSION_CHARSET_NAME));
