...
public class TransactionResubmissionDialog implements Initializable {
    ...
    @FXML private VBox transactions;

    public TransactionResubmissionDialog(
            AccountManager accountManager, ConsoleManager consoleManager) {
    ...
    public void open(final MouseEvent mouseEvent) {
        ...
        try {
            resubmitTransactionDialog =
                    FXMLLoader.load(getClass().getResource("TransactionResubmissionDialog.fxml"));
        } catch (IOException e) {
    ...
    private void displayTransactions() {
        addHeaderForTable();
        final Optional<AccountDTO> first =
                accountManager.getAccounts().stream().filter(AccountDTO::isActive).findFirst();
        final String publicAddress;
        if (first.isPresent()) {
            publicAddress = first.get().getPublicAddress();
            for (SendTransactionDTO unsentTransaction :
                    accountManager.getTimedOutTransactions(publicAddress)) {
                HBox row = new HBox();
                row.setSpacing(10);
                ...
                resubmitTransaction.setOnMouseClicked(
                        event -> {
                            close(event);
