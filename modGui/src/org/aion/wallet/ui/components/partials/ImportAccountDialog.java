...
public class ImportAccountDialog implements Initializable {
    ...
    private AccountDTO getAccountFromKeyStore(final boolean shouldKeep) {
        ...
        if (!password.isEmpty() && keystoreFile != null) {
            try {
                ...
                consoleManager
                    .addLog("Keystore could not be imported", ConsoleManager.LogType.ACCOUNT,
    ...
    private AccountDTO getAccountFromPrivateKey(final boolean shouldKeep) {
        ...
        if (password != null && !password.isEmpty() && privateKey != null && !privateKey
            .isEmpty()) {
            byte[] raw = Hex
                .decode(privateKey.startsWith("0x") ? privateKey.substring(2) : privateKey);
            if (raw == null) {
            ...
            try {
                ...
                consoleManager
                    .addLog("Private key could not be imported", ConsoleManager.LogType.ACCOUNT,
                        ConsoleManager.LogLevel.WARNING);
                LOG.error(e.getMessage(), e);
                displayError(e.getMessage());
                return null;
    ...
    public void open(MouseEvent mouseEvent) {
        ...
        try {
            FXMLLoader loader = new FXMLLoader(
                (getClass().getResource("ImportAccountDialog.fxml")));
        ...
        Scene secondScene = new Scene(pane, importAccountDialog.getPrefWidth(),
    ...
    private void radioButtonChanged(ObservableValue<? extends Toggle> ov, Toggle oldToggle,
        Toggle newToggle) {
