...
public class UnixProcessTerminator {
     ...
     *
     * <p>Specifically, send SIGTERM to that process, then poll the UNIX ps program to verify that
     ...
     */
    public void terminateAndAwait(KernelInstanceId kernelInstanceId) throws KernelControlException {
        ...
        try {
            ...
            throw new KernelControlException("Error while trying to terminate kernel process.", ex);
        }
    ...
    private boolean blockUntilProcessDeadOrTimeout(long unixPid, Duration timeout)
    ...
    /**
     * Put all the work of actually running OS commands into this class, so that
     * UnixProcessTerminator unit tests can test the rest of the code in isolation by mocking this
     * class. Logic of this class will be covered by integ tests.
     */
    @VisibleForTesting
    static class UnixCommandRunner {
        int sendSigterm(long unixPid) throws IOException, InterruptedException {
            ProcessBuilder processBuilder =
                    new ProcessBuilder()
                            .command("kill", String.valueOf(unixPid))
        ...
        int sendSigKill(long unixPid) throws IOException, InterruptedException {
            ProcessBuilder processBuilder =
                    new ProcessBuilder()
                            .command("kill", "-9", String.valueOf(unixPid))
                            .redirectOutput(ProcessBuilder.Redirect.INHERIT)
                            .redirectError(ProcessBuilder.Redirect.INHERIT);
            return processBuilder.start().waitFor();
        }
        ...
        List<String> callPs(long pid) throws IOException, InterruptedException {
            final String[] command =
                    new String[] {"ps", "-o", "pid=", "--pid", String.valueOf(pid)};
