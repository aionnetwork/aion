...
public class TransactionProcessor extends AbstractAionApiClient {
    ...
    public Future<?> processTxnsFromBlockAsync(final BlockDTO lastSafeBlock,
        final Set<String> addresses) {
        return backgroundExecutor
            .submit(() -> processTransactionsFromBlock(lastSafeBlock, addresses));
    }
    ...
    private void processTransactionsFromBlock(final BlockDTO lastSafeBlock,
        final Set<String> addresses) {
        if (!apiIsConnected()) {
            LOG.trace(
        ...
        LOG.debug("Processing transactions from block: {} to block: {}, for addresses: {}",
            previousSafe, latest, addresses);
        if (previousSafe > 0) {
        ...
        for (long i = latest; i > previousSafe; i -= BLOCK_BATCH_SIZE) {
            List<Long> blockBatch = LongStream.iterate(i, j -> j - 1).limit(BLOCK_BATCH_SIZE)
        ...
        if (newSafeBlockNumber > 0) {
            ...
            for (String address : addresses) {
                accountManager.updateLastSafeBlock(address,
                    new BlockDTO(newSafe.getNumber(), newSafe.getHash().toBytes()));
            }
        }
    ...
    private void removeTransactionsFromBlock(final Set<String> addresses, final long previousSafe) {
        for (String address : addresses) {
            final List<TransactionDTO> txs = new ArrayList<TransactionDTO>(
                accountManager.getTransactions(address));
    ...
    private TransactionResponseDTO sendTransactionInternal(final SendTransactionDTO dto) {
        ...
        consoleManager.addLog("Sending transaction", ConsoleManager.LogType.TRANSACTION,
    ...
    private TransactionResponseDTO mapTransactionResponse(final MsgRsp response) {
        return new TransactionResponseDTO(response.getStatus(), response.getTxHash(),
            response.getError());
    }
    ...
    private void processTransactionsFromOldestRegisteredSafeBlock() {
        ...
        final BlockDTO oldestSafeBlock = accountManager
            .getOldestSafeBlock(addresses, nullSafeBlockFilter);
