...
public class ReceiveControllerIntegTest extends ApplicationTest {
    ...
    public void start(Stage stage) throws Exception {
        ...
        cf = new ControllerFactory().withEventBusRegistry(ebr);
        FXMLLoader loader =
                new FXMLLoader(
    ...
    public void testInitial() {
        ...
        clickOn(
                "#copyToClipboardButton"); // nothing to really verify, just click it to make sure
                                           // no exceptions
    }
    ...
    public void testInitialThenChangeToSomeAccount() throws Exception {
        ...
        RGBLuminanceSource rgbSource =
                new RGBLuminanceSource(image.getWidth(), image.getHeight(), rgb);
        ...
        Platform.runLater(
                () ->
                        assertThat(
                                Clipboard.getSystemClipboard().getContent(DataFormat.PLAIN_TEXT),
                                is(account.getPublicAddress())));
    ...
    public void testSomeAccountThenChangeToOtherAccount() throws Exception {
        AccountDTO account =
                new AccountDTO("anyName", "anyAddr", "anyBalance", "anyCurrency", false, 0);
        EventBusRegistry.INSTANCE
                .getBus(AccountEvent.ID)
                .post(new AccountEvent(AccountEvent.Type.CHANGED, account));
