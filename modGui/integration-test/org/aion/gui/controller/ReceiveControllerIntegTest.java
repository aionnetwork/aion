...
public class ReceiveControllerIntegTest extends ApplicationTest {
    ...
    public void testInitial() {
        ...
        clickOn("#copyToClipboardButton"); // nothing to really verify, just click it to make sure no exceptions
    }
    ...
    public void testInitialThenChangeToSomeAccount() throws  Exception {
        ...
        RGBLuminanceSource rgbSource = new RGBLuminanceSource(image.getWidth(), image.getHeight(), rgb);
        ...
        Platform.runLater(() -> assertThat(Clipboard.getSystemClipboard().getContent(DataFormat.PLAIN_TEXT),
                is(account.getPublicAddress())));
    ...
    public void testSomeAccountThenChangeToOtherAccount() throws Exception {
        ...
        EventBusRegistry.INSTANCE.getBus(AccountEvent.ID).post(new AccountEvent(AccountEvent.Type.CHANGED, otherAccount));
    ...
    public void testSomeAccountThenLockAccount() {
        ...
        EventBusRegistry.INSTANCE.getBus(AccountEvent.ID).post(new AccountEvent(AccountEvent.Type.LOCKED, account));
        assertThat( ((ImageView)receivePaneView.lookup("#qrCode")).getImage(), is(nullValue()));
        assertThat(
                ((TextArea)receivePaneView.lookup("#accountAddress")).getText(),
