 ...
 * @author yao
 */
public abstract class TransactionExecThread<
        PS extends IPendingStateInternal, TX extends ITransaction> {
    ...
    public Future<List<TX>> submitTransaction(TX tx) {
        Future<List<TX>> txListFuture =
                txExec.submit(
                        () -> {
                            LOG.debug(
                                    "TransactionExecThread.broadcastTransaction: " + tx.toString());
