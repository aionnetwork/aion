 ...
 * <p>
 * - NULL (represented as the empty string) - A two-item array [ key, value ] (1 key for 2-item
 * array) - A 17-item array [ v0 ... v15, vt ] (16 keys for 17-item array)
 * <p>
 * The idea is that in the event that there is a long path of nodes each with only one element, we
 * shortcut the descent by setting up a [ key, value ] node, where the key gives the hexadecimal
 ...
 * R / \ /   \ N     N / \   / \ L   L L   L
 * <p>
 * Also, we add another conceptual change: internal nodes can no longer have values, only leaves
 ...
 * <p>
 * Note that when updating a trie, you will need to store the key/value pair (keccak(x), x) in a
