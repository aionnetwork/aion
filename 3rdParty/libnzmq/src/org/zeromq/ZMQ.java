...
public class ZMQ {
    ...
    /**
     * EVENT_CONNECTED: connection established. The EVENT_CONNECTED event
     * triggers when a connection has been established to a remote peer. This
     * can happen either synchronous or asynchronous. Value is the FD of the
     * newly connected socket.
     */
    public static final int EVENT_CONNECTED = 1;
    /**
     * EVENT_CONNECT_DELAYED: synchronous connect failed, it's being polled. The
     * EVENT_CONNECT_DELAYED event triggers when an immediate connection attempt
     * is delayed and its completion is being polled for. Value has no meaning.
     */
    ...
    /**
     * EVENT_BIND_FAILED: socket could not bind to an address. The
     * EVENT_BIND_FAILED event triggers when a socket could not bind to a given
     * interface. Value is the errno generated by the bind call.
     */
    public static final int EVENT_BIND_FAILED = 16;

    /**
     * EVENT_ACCEPTED: connection accepted to bound interface. The
     * EVENT_ACCEPTED event triggers when a connection from a remote peer has
     * been established with a socket's listen address. Value is the FD of the
     * accepted socket.
     */
    public static final int EVENT_ACCEPTED = 32;
    /**
     * EVENT_ACCEPT_FAILED: could not accept client connection. The
     * EVENT_ACCEPT_FAILED event triggers when a connection attempt to a
     ...
     * connection's underlying descriptor has been closed. Value is the former
     * FD of the for the closed socket. FD has been closed already!
     */
     ...
     * Implementation note: ONLY FOR IPC SOCKETS. Value is the errno generated
     * by unlink.
     */
    public static final int EVENT_CLOSE_FAILED = 256;
    /**
     * EVENT_DISCONNECTED: broken session. The EVENT_DISCONNECTED event triggers
     * when the stream engine (tcp and ipc specific) detects a corrupted /
     * broken session. Value is the FD of the socket.
     */
    public static final int EVENT_DISCONNECTED = 512;
    /**
     * EVENT_MONITOR_STOPPED: monitor has been stopped. The
     * EVENT_MONITOR_STOPPED event triggers when the monitor for a socket is
     * stopped.
     */
    public static final int EVENT_MONITOR_STOPPED = 1024;

    /**
     * EVENT_ALL: all events known by the java binding. The EVENT_ALL constant
     * can be used to set up a monitor for all events known by the java binding.
     * One could add more flags
     */
    ...
    /**
     * Starts the built-in 0MQ proxy in the current application thread. The
     * proxy connects a frontend socket to a backend socket. Conceptually, data
     * flows from frontend to backend. Depending on the socket types, replies
     * may flow in the opposite direction. The direction is conceptual only; the
     * proxy is fully symmetric and there is no technical difference between
     * frontend and backend.
     *
     * Before calling ZMQ.proxy() you must set any socket options, and connect
     * or bind both frontend and backend sockets. The two conventional proxy
     * models are:
     *
     * ZMQ.proxy() runs in the current thread and returns only if/when the
     * current context is closed.
     *
     * @param frontend ZMQ.Socket
     * @param backend ZMQ.Socket
     * @param capture If the capture socket is not NULL, the proxy shall send
     * all messages, received on both frontend and backend, to the capture
     * socket. The capture socket should be a ZMQ_PUB, ZMQ_DEALER, ZMQ_PUSH, or
     * ZMQ_PAIR socket.
     * @since 3.2.2
     */
    public static void proxy(Socket frontend, Socket backend, Socket capture) {
        ...
        if (ZMQ.version_full() < ZMQ.make_version(3, 2, 2)) {
            System.err.println("ZMQ proxy need ver > 3.2.2.  current ver is :" + ZMQ.version_full());
     ...
     *
     * @param ioThreads Number of threads to use, usually 1 is sufficient for
     * most use cases.
     * @return the Context
     */
...
//            }
        ...
        /**
         * Returns the endpoint address represented by the last bind operation.
         * This is useful for determining the port that was assigned by the
         * operating system in instances where the bind address contains
         * wildcards, such as tcp://*:*.
         *
         ...
         *
         * @param timeout Timeout for receive operation in milliseconds. Default
         * -1 (infinite)
         */
         ...
         * that amount of time before returning with an EAGAIN error.
         *
         * @param timeout Timeout for send operation in milliseconds. Default -1
         * (infinite)
         */
         ...
         * than held in memory.
         *
         * @param swap The value of 'ZMQ_SWAP' defines the maximum size of the
         * swap space in bytes.
         */
         ...
         * default.
         *
         * @param optVal The value of 'ZMQ_TCP_KEEPALIVE' to turn TCP keepalives
         * on (1) or off (0).
         */
         ...
         *
         * @param optVal The value of 'ZMQ_TCP_KEEPALIVE_CNT' defines the number
         ...
         * default value -1 will skip all overrides and do the OS default.
         *
         * @param optVal The value of 'ZMQ_TCP_KEEPALIVE_INTVL' defines the
         ...
         *
         * @param optVal The value of 'ZMQ_TCP_KEEPALIVE_IDLE' defines the
         * interval between the last data packet sent over the socket and the
         * first keepalive probe. Unit is OS dependant.
         */
         ...
         * interval determines the maximum time in seconds (before version
         * 3.0.0) or milliseconds (version 3.0.0 and after) that a receiver can
         * be absent from a multicast group before unrecoverable data loss will
         * occur.
         *
         * CAUTION: Exercise care when setting large recovery intervals as the
        ...
        /**
         * The 'ZMQ_IPV4ONLY' option shall set the underlying native socket
         * type. An IPv6 socket lets applications connect to and accept
         * connections from both IPv4 and IPv6 hosts.
         *
         * @param v4only A value of true will use IPv4 sockets, while the value
         * of false will use IPv6 sockets
         */
         ...
         * encountered.
         *
         * @param mandatory A value of false is the default and discards the
         * message silently when it cannot be routed. A value of true returns an
         * EHOSTUNREACH error code if the message cannot be routed.
         */
         ...
         * unsubscriptions.
         *
         * @param verbose A value of false is the default and passes only new
         * subscription messages to upstream. A value of true passes all
        ...
        /**
         * Sets if the socket is for a server using the PLAIN security
         * mechanism.
         *
         ...
         *
         * @param domain For NULL security (the default on all tcp://
         * connections), ZAP authentication only happens if you set a non-empty
         * domain. For PLAIN and CURVE security, ZAP requests are always made,
         * if there is a ZAP handler present. See http://rfc.zeromq.org/spec:27
         * for more details.
         */
         ...
         *
         * <p>
         * True means the socket will act as CURVE server, while false means the
         * socket will not act as CURVE server, and its security role then
         * depends on other option settings. Setting this to false shall reset
         * the socket security to NULL. When you set this you must also set the
         * server's secret key using the {@link #setCurveSecretKey(byte[])}
         * function.</p>
         *
         ...
         *
         * <p>
         * You must set this on CURVE client sockets. You can provide the key as
         ...
         *
         * <p>
         * You must set this on both CURVE client and server sockets. You can
        ...
        /**
         * Sets whether socket should keep only last received/to be sent message
         * in its inbound/outbound queue.
         *
         * @param conflate A value of false is the default which means socket
         * preserves all messages with respect to the RECVHWM and SENDHWM
         * options. A value of true means only last message is kept ignoring the
         * RECVHWM and SENDHWM options.
         * @since 4.0.0
         */
        ...
        /**
         * Indicate whether socket should keep only last received/to be sent
         * message in its inbound/outbound queue.
         *
         * @return true if should keep only last received/to be sent message in
        ...
        /**
         * Indicate whether socket should only queue messages to completed
         * connections.
         *
        ...
        /**
         * Sets whether socket should only queue messages to completed
         * connections.
         *
         * @param immediate A value of false is the default which means socket
         * will not queue messages to to incomplete connections. This will cause
         * the socket to block if there are no other connections, but will
         ...
         * when a new connection is made or accepted. This may be set on REQ,
         * DEALER, or ROUTER sockets connected to a ROUTER socket. The
         * application must filter such empty messages.
         *
         * @param isProbeRouter if {@code true}, the socket will automatically
         * send an empty message when a new connection is made or accepted; if
         ...
         * connections. The main difference from bindToRandomPort is that it
         * allows system to pick up the right port. For this reason return type
         * is a string and its responsibility of a caller to parse into the
         * right integer type (e.g. for vmci transport it should be long).
         *
         * @param addr the endpoint to bind to.
         */
         ...
         *
         * @param addr the endpoint to receive events from. (must be inproc
         * transport)
         ...
         * @param len max bytes to write to buffer. If len is smaller than the
    ...
    public static class Poller {
         ...
         * <p>
         * Since ZeroMQ 3.0, the timeout parameter is in <i>milliseconds</i>,
         * but prior to this the unit was
         * <i>microseconds</i>.
         *
         * @param items an array of PollItem to poll.
         * @param timeout the maximum timeout in milliseconds/microseconds (see
         * above).
         * @return how many objects where signalled by poll.
         * @see <a href="http://api.zeromq.org/2-1:zmq-poll">2.1 docs</a>
         * @see <a href="http://api.zeromq.org/3-0:zmq-poll">3.0 docs</a>
    ...
    public static class Event {
        ...
        /**
         * Get the address. For libzmq versions 3.2.x the address will be an
         * empty string.
         *
         * @return
         */
        ...
        /**
         * Receive an event from a monitor socket. For libzmq versions 3.2.x the
         * address will be an empty string.
         *
        ...
        /**
         * Receive an event from a monitor socket. Does a blocking recv. For
         * libzmq versions 3.2.x the address will be an empty string.
         *
         * @param socket the socket
    ...
    public static class Curve {
         ...
         *
         * <p>
         * The keys are encoded using {@link #z85Encode}.</p>
         *
        ...
        /**
         * The function shall decode given key encoded as Z85 string into byte
         * array.
         *
         * <p>
         * The decoding shall follow the ZMQ RFC 32 specification.</p>
         *
         ...
         *
         * <p>
         * The encoding shall follow the ZMQ RFC 32 specification.</p>
         *
