 ...
 * last M blocks
 *
 * This design has a bias toward the lower end of the threshold of nrg prices required for txns to
 * be accepted into blocks (heuristically)
 *
 * A transaction based design alternatively has been observed in the wild to have a positive
 * feedback effect where large numbers of people following the recommendation will tend the
 * recommendation upward
 *
 * This class is NOT thread-safe Policy: holder class (NrgOracle) should provide any concurrency
...
public class NrgBlockPriceAveraging extends NrgPriceAdvisor<AionBlock, AionTransaction> {
    ...
    public NrgBlockPriceAveraging(long defaultPrice, long maxPrice, int windowSize,
        int percentile) {
    ...
    public long computeRecommendation() {
        ...
        try {
            ...
            LOG.error(
                "<nrg-price-strategy: unable to compute mean. returning price at recommendationIndex.",
                e);
        }
