 ...
 *
 * <p>NOT TESTED. DON'T USE.
 *
 * @author ali sharif
 */
public class NrgBlockPriceAveraging extends NrgPriceAdvisor<AionBlock> {
    ...
    public NrgBlockPriceAveraging(
            long defaultPrice, long maxPrice, int windowSize, int percentile) {
    ...
    public long computeRecommendation() {
        ...
        if (isHungry() || windowSize == 1) return defaultPrice;

        // let the backing syncronized collection do the locking for us.
        Long[] blkPrice = blkPriceQ.toArray(new Long[blkPriceQ.size()]);
        // n*log(n) sort
        // (https://docs.oracle.com/javase/9/docs/api/java/util/Arrays.html#sort-long:A-)
