...
public class UndertowRpcServer extends RpcServer {
    ...
    private final Map<HttpString, String> CORS_HEADERS =
            Map.of(
                    HttpString.tryFromString("Access-Control-Allow-Origin"), corsOrigin,
                    HttpString.tryFromString("Access-Control-Allow-Headers"),
    ...
    private UndertowRpcServer(Builder builder) {
        super(builder);
        // writes to System.error. Rationale: the alternative is to write to the slf4j facade and
        // then manually
        // hook up all the possible loggers defined (now and in future) by this library
        // through our logback logger; this risks missing potentially important debug information
        // from the library
    ...
    private SSLContext sslContext() throws Exception {
        try {
            ...
            for (char c : sslCertPass) c = '\0'; // NUL
    ...
    public void start() {
        try {
            ...
            if (isQueueBounded) {
                requestLimiting =
                        new RequestLimitingConfiguration(
                                true, effectiveWorkerThreadCount, getRequestQueueSize().get());
            }

            AionUndertowRpcHandler rpcHandler =
                    new AionUndertowRpcHandler(corsEnabled, CORS_HEADERS, rpcProcessor);
            ...
            LOG.info(
                    "<rpc-server - (UNDERTOW) started on {}://{}:{}>",
            ...
            LOG.debug(
                    "CORS: {}",
                    corsEnabled
                            ? "Enabled; Allowed Origins = \"" + corsOrigin + "\""
                            : "Not Enabled");
