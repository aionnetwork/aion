...
public abstract class Api<B extends AbstractBlock<?, ?>> {
    ...
    public boolean unlockAccount(final String _address, final String _password,
        final int _duration) {
        return this.ACCOUNT_MANAGER.unlockAccount(Address.wrap(_address), _password, _duration);
    }

    public boolean unlockAccount(final Address _address, final String _password,
        final int _duration) {
    ...
    public Map<String, CompiledContr> contract_compileSolidity(final String _contract) {
        try {
            ...
            Compiler.Result res = solc
                .compile(_contract.getBytes(), Compiler.Options.ABI, Compiler.Options.BIN);
            if (res.isFailed()) {
            ...
            Iterator<Entry<String, CompilationResult.Contract>> entries = result.contracts
                .entrySet().iterator();
