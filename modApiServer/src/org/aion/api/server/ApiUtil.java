...
public class ApiUtil {
    ...
    public static byte[] toReturnHeader(int vers, int retCode, byte[] hash) {
        ...
        return ByteBuffer.allocate(HASH_LEN + RETHEADER_LEN).put((byte) vers).put((byte) retCode)
            .put((byte) 1)
    ...
    public static byte[] toReturnHeader(int vers, int retCode, byte[] hash, byte[] error) {
        ...
        if (error.length == 0) {
            return ByteBuffer.allocate(HASH_LEN + RETHEADER_LEN + 1).put((byte) vers)
                .put((byte) retCode).put((byte) 1)
                .put(hash, 0, hash.length).put((byte) 0).array();
        } else {
            return ByteBuffer.allocate(HASH_LEN + RETHEADER_LEN + 1 + error.length).put((byte) vers)
                .put((byte) retCode).put((byte) 1)
                .put(hash, 0, hash.length).put((byte) error.length).put(error, 0, error.length)
                .array();
        }
    }
    ...
    public static byte[] toReturnHeader(int vers, int retCode, byte[] hash, byte[] error,
        ...
        if (error.length == 0) {
            return ByteBuffer.allocate(HASH_LEN + RETHEADER_LEN + 1 + result.length)
                .put((byte) vers).put((byte) retCode)
                .put((byte) 1).put(hash, 0, hash.length).put((byte) 0).put(result, 0, result.length)
                .array();
        } else {
            return ByteBuffer.allocate(HASH_LEN + RETHEADER_LEN + 1 + error.length + result.length)
                .put((byte) vers).put((byte) retCode)
                .put((byte) 1).put(hash, 0, hash.length).put((byte) error.length)
                .put(error, 0, error.length).put(result, 0, result.length).array();
        }
    }

    public static byte[] toReturnHeader(int vers, int retCode) {

        return ByteBuffer.allocate(RETHEADER_LEN).put((byte) vers).put((byte) retCode).put((byte) 0)
            .array();
    }
    ...
    public static byte[] combineRetMsg(byte[] header, byte[] body) {
        ...
        return ByteBuffer.allocate(header.length + body.length).put(header, 0, header.length)
    ...
    public static byte[] combineRetMsg(byte[] header, byte body) {
        ...
        return ByteBuffer.allocate(header.length + 1).put(header, 0, header.length).put(body)
            .array();
    }
    ...
    public static byte[] toReturnEvtHeader(byte vers, byte[] ecb) {
        return ByteBuffer.allocate(RETHEADER_LEN + ecb.length).put(vers).put((byte) 106)
            .put((byte) 0)
