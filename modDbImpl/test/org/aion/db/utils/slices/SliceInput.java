...
public final class SliceInput extends InputStream implements DataInput {
    ...
    /**
     * Returns the number of readable bytes which is equal to {@code (this.slice.length() -
     * this.position)}.
     */
    @Override
    ...
    /**
     * Gets a byte at the current {@code position} and increases the {@code position} by {@code 1}
     * in this buffer.
     *
    ...
    /**
     * Gets a 16-bit short integer at the current {@code position} and increases the {@code
     * position} by {@code 2} in this buffer.
     *
    ...
    /**
     * Gets an unsigned 32-bit integer at the current {@code position} and increases the {@code
     * position} by {@code 4} in this buffer.
     *
     * @throws IndexOutOfBoundsException if {@code this.available()} is less than {@code 4}
     */
     ...
     * position} and increases the {@code position} by the number of the transferred bytes (= {@code
     * length}). The returned buffer's {@code position} and {@code writerIndex} are {@code 0} and
     ...
     *     {@code 0}, if {@code length} is greater than {@code this.available()}, or if {@code
     *     destinationIndex + length} is greater than {@code destination.length}
     */
    ...
    /**
     * Transfers this buffer's data to the specified destination starting at the current {@code
     * position} until the destination becomes non-writable, and increases the {@code position} by
     * the number of the transferred bytes. This method is basically same with {@link
     * #readBytes(Slice, int, int)}, except that this method increases the {@code writerIndex} of
     * the destination by the number of the transferred bytes while {@link #readBytes(Slice, int,
     * int)} does not.
     *
     * @throws IndexOutOfBoundsException if {@code destination.writableBytes} is greater than {@code
     *     this.available()}
     */
     ...
     * Transfers this buffer's data to the specified destination starting at the current {@code
     * position} and increases the {@code position} by the number of the transferred bytes (= {@code
     * length}). This method is basically same with {@link #readBytes(Slice, int, int)}, except that
     * this method increases the {@code writerIndex} of the destination by the number of the
     * transferred bytes (= {@code length}) while {@link #readBytes(Slice, int, int)} does not.
     *
     * @throws IndexOutOfBoundsException if {@code length} is greater than {@code this.available()}
     *     or if {@code length} is greater than {@code destination.writableBytes}
     */
     ...
     * @throws IndexOutOfBoundsException if the specified {@code destinationIndex} is less than
     *     {@code 0}, if {@code length} is greater than {@code this.available()}, or if {@code
     *     destinationIndex + length} is greater than {@code destination.capacity}
     */
    ...
    /**
     * Transfers this buffer's data to the specified destination starting at the current {@code
     * position} until the destination's position reaches its limit, and increases the {@code
     * position} by the number of the transferred bytes.
     *
     * @throws IndexOutOfBoundsException if {@code destination.remaining()} is greater than {@code
     *     this.available()}
     */
     ...
     * method is identical to {@code buf.toByteBuffer(buf.position(), buf.available()())}. This
     * method does not modify {@code position} or {@code writerIndex} of this buffer.
     */
     ...
     * This method is identical to {@code buf.toString(buf.position(), buf.available()(),
     * charsetName)}. This method does not modify {@code position} or {@code writerIndex} of this
     * buffer.
     *
